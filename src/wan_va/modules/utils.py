...
def load_transformer(
    ...
    **kwargs
    ...
    materialize_missing_modules = bool(
        kwargs.pop("materialize_missing_modules", False)
    )
    if materialize_missing_modules:
        kwargs["low_cpu_mem_usage"] = False
        keep_fp32 = WanTransformer3DModel._keep_in_fp32_modules
        # Diffusers disallows low_cpu_mem_usage=False together with this class
        # hint.  Temporarily remove the loading hint; FP32LayerNorm still performs
        # its computation in float32 by construction.
        WanTransformer3DModel._keep_in_fp32_modules = None
        try:
            ...
            WanTransformer3DModel._keep_in_fp32_modules = keep_fp32
    else:
        model = WanTransformer3DModel.from_pretrained(
            transformer_path,
            torch_dtype=torch_dtype,
