...
class ActionHeadAttention(nn.Module):
    """Pre-normalized attention with independent query and KV projections."""

    def __init__(self, dim, num_heads, eps=1e-6, dropout=0.0):
        super().__init__()
        if dim % num_heads != 0:
            raise ValueError(f"action head dim={dim} must be divisible by heads={num_heads}")
        self.num_heads = int(num_heads)
        self.head_dim = dim // num_heads
        self.to_q = nn.Linear(dim, dim)
        self.to_k = nn.Linear(dim, dim)
        self.to_v = nn.Linear(dim, dim)
        self.q_norm = nn.RMSNorm(dim, eps=eps)
        self.k_norm = nn.RMSNorm(dim, eps=eps)
    ...
    def forward(self, query, key_value, key_mask=None):
        ...
        q = self.q_norm(self.to_q(query)).view(
            batch, query_len, self.num_heads, self.head_dim
        ).transpose(1, 2)
        k = self.k_norm(self.to_k(key_value)).view(
            batch, key_len, self.num_heads, self.head_dim
        ).transpose(1, 2)
        v = self.to_v(key_value).view(
            batch, key_len, self.num_heads, self.head_dim
        ).transpose(1, 2)
        attention_mask = None
        if key_mask is not None:
            attention_mask = key_mask[:, None, None, :].to(dtype=torch.bool)
        output = F.scaled_dot_product_attention(
            q, k, v, attn_mask=attention_mask, dropout_p=0.0
        )
        output = output.transpose(1, 2).reshape(batch, query_len, dim)
        return self.out(output)


class WanActionHeadBlock(nn.Module):
    """Small action block: action Q attends to projected video + action/state KV."""
    ...
    def __init__(
        ...
        super().__init__()
        self.query_norm = FP32LayerNorm(action_dim, eps, elementwise_affine=True)
        self.action_kv_norm = FP32LayerNorm(action_dim, eps, elementwise_affine=True)
        self.video_norm = FP32LayerNorm(video_dim, eps, elementwise_affine=True)
        self.video_mlp = nn.Sequential(
            nn.Linear(video_dim, action_dim),
            nn.SiLU(),
            nn.Linear(action_dim, action_dim),
        )
        self.cross_modal_attn = ActionHeadAttention(
            action_dim, num_heads, eps=eps, dropout=dropout
        )
        self.text_norm = FP32LayerNorm(action_dim, eps, elementwise_affine=True)
        self.text_attn = ActionHeadAttention(
            action_dim, num_heads, eps=eps, dropout=dropout
        )
        self.ffn_norm = FP32LayerNorm(action_dim, eps, elementwise_affine=False)
        self.ffn = FeedForward(
            action_dim, inner_dim=ffn_dim, activation_fn="gelu-approximate"
        )
        self.scale_shift_table = nn.Parameter(
    ...
    def forward(
        ...
        *,
        ...
        modulation = self.scale_shift_table[None] + timestep_projection.float()
        ...
        ff_shift, ff_scale, ff_gate = (
            ff_shift.squeeze(1), ff_scale.squeeze(1), ff_gate.squeeze(1)
        )

        query = (
            self.query_norm(action_states.float()) * (1.0 + scale) + shift
        ).type_as(action_states)
        video_kv = self.video_mlp(self.video_norm(video_states.float()).type_as(video_states))
        action_kv_inputs = action_states
        if conditioning_states is not None:
            action_kv_inputs = torch.cat(
                [action_kv_inputs, conditioning_states], dim=1
            )
        action_kv = self.action_kv_norm(action_kv_inputs.float()).type_as(action_states)
        key_value = torch.cat([video_kv, action_kv], dim=1)
        if action_key_mask is not None:
            video_mask = torch.ones(
                (action_key_mask.shape[0], video_kv.shape[1]),
                dtype=torch.bool,
                device=action_key_mask.device,
            )
            kv_mask = torch.cat([video_mask, action_key_mask], dim=1)
        else:
            kv_mask = None
        action_states = (
            action_states.float()
            + self.cross_modal_attn(query, key_value, kv_mask).float() * gate
        ).type_as(action_states)

        text_query = self.text_norm(action_states.float()).type_as(action_states)
        action_states = action_states + self.text_attn(
            text_query, text_states, text_key_mask
        )
        ff_input = (
            self.ffn_norm(action_states.float()) * (1.0 + ff_scale) + ff_shift
        ).type_as(action_states)
        action_states = (
            action_states.float() + self.ffn(ff_input).float() * ff_gate
        ).type_as(action_states)
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
                 ...
                 action_head_hidden_size=768,
                 action_head_num_attention_heads=12,
                 action_head_ffn_dim=3072,
                 action_head_num_layers=None,
        ...
        if self.action_head_type not in {"shared", "separate"}:
        ...
        self._action_video_context_cache = {}
        self._action_state_cache = {}
        if self.action_head_type == "separate":
            action_layers = int(action_head_num_layers or num_layers)
            if action_layers != num_layers:
                raise ValueError(
                    "Separate action head must have the same number of layers as the video "
                    f"backbone ({num_layers}), got {action_layers}"
                )
            action_hidden = int(action_head_hidden_size)
            self.action_head_embedder = nn.Linear(action_dim, action_hidden)
            self.action_head_condition_embedder = WanTimeTextImageEmbedding(
                dim=action_hidden,
                time_freq_dim=freq_dim,
                time_proj_dim=action_hidden * 6,
                text_embed_dim=text_dim,
                pos_embed_seq_len=pos_embed_seq_len,
            )
            self.action_blocks = nn.ModuleList(
                [
                    WanActionHeadBlock(
                        video_dim=inner_dim,
                        action_dim=action_hidden,
                        ffn_dim=int(action_head_ffn_dim),
                        num_heads=int(action_head_num_attention_heads),
                        eps=eps,
            ...
            self.action_head_norm_out = FP32LayerNorm(
                action_hidden, eps, elementwise_affine=False
            )
            self.action_head_proj_out = nn.Linear(action_hidden, action_dim)
            self.action_head_scale_shift_table = nn.Parameter(
                torch.randn(1, 2, action_hidden) / action_hidden**0.5
            )
    ...
    def clear_cache(self, cache_name):
        for block in self.blocks:
            if hasattr(block.attn1, "attn_caches") and block.attn1.attn_caches is not None:
                block.attn1.attn_caches.pop(cache_name, None)
        self._action_video_context_cache.pop(cache_name, None)
        self._action_state_cache.pop(cache_name, None)

    def create_empty_cache(self, cache_name, attn_window,
                           latent_token_per_chunk, action_token_per_chunk,
                           device, dtype, batch_size):
        total_tolen = latent_token_per_chunk + action_token_per_chunk
        for block in self.blocks:
    ...
    def forward_train(self, input_dict):
        export_mask = input_dict.get("export_mask", False)
        mask_output_dir = input_dict.get("mask_output_dir")
        ...
        input_dict['action_dict']['state']['value'] = input_dict['action_dict']['state']['value'].to(torch.bfloat16)
        input_dict['action_dict']['state']['value'] = input_dict['action_dict']['state']['value'].to(torch.bfloat16)
        ...
        state_dict = action_dict["state"]
        state_hidden_states = self._input_embed(state_dict["value"], input_type='action').flatten(0, 1)[None]
        text_hidden_states = self._input_embed(latent_dict["text_emb"], input_type='text')

        action_head_states = action_head_temb = action_head_timestep_proj = None
        action_head_conditioning = None
        action_head_key_mask = None
        action_token_count = action_dict['noisy_latents'].shape[-3] * action_dict['noisy_latents'].shape[-2] * action_dict['noisy_latents'].shape[-1]
        if self.action_head_type == "separate":
            action_head_actions = rearrange(
                action_dict['noisy_latents'], 'b c f h w -> b (f h w) c'
            )
            action_head_state = rearrange(
                state_dict['value'], 'b c f h w -> b (f h w) c'
            )
            action_head_states = self.action_head_embedder(action_head_actions)
            action_head_conditioning = self.action_head_embedder(action_head_state)
            action_head_text = self.action_head_condition_embedder.text_embedder(
                latent_dict["text_emb"]
            )
            action_head_key_mask = torch.cat(
                [
                    torch.ones(
                        (batch_size, action_head_actions.shape[1]),
                        device=action_head_actions.device,
                        dtype=torch.bool,
                    ),
                    state_dict["mask"].to(device=action_head_actions.device, dtype=torch.bool),
                ],
                dim=1,
            )
            action_steps = action_dict['timesteps']
            action_head_temb_action, action_head_proj_action = self._action_head_time_embed(
                action_steps,
                action_dict['noisy_latents'].shape[-2],
                action_dict['noisy_latents'].shape[-1],
                action_head_states.dtype,
            )
            action_head_temb = action_head_temb_action
            action_head_timestep_proj = action_head_proj_action
        ...
        state_grid_id = state_dict["grid_id"].permute(1, 0, 2).flatten(1)[None]
        full_grid_id = torch.cat([latent_grid_id, action_grid_id, state_grid_id], dim=2)

        rotary_emb = self.rope(full_grid_id)[:, :, None] 

        latent_time_steps = latent_dict['timesteps'].flatten(0, 1)[None]
        ...
        action_temb, action_timestep_proj = self._time_embed(action_time_steps,
                        ...
                        dtype=hidden_states.dtype, 
                        action_mode=True)
        state_time_steps = state_dict["timesteps"].flatten(0, 1)[None]
        state_temb, state_timestep_proj = self._time_embed(
                        state_time_steps,
                        state_dict["value"].shape[-2],
                        state_dict["value"].shape[-1],
                        dtype=hidden_states.dtype,
                        action_mode=True)
        temb = torch.cat([latent_temb, action_temb, state_temb], dim=1)
        timestep_proj = torch.cat(
            [latent_timestep_proj, action_timestep_proj, state_timestep_proj], dim=1
        )
        ...
        split_list = [latent_hidden_states.shape[1], 
                      ...
                      state_hidden_states.shape[1],
                      padded_length]

        if self.blocks[0].attn_mode == "flex":
            FlexAttnMaskManager.init_masks(latent_dict['noisy_latents'].shape,
                                           ...
                                           state_shape=state_dict["value"].shape,
                                           state_mask=state_dict.get("mask"),
                                           )

        latent_tokens_per_sample = latent_hidden_states.shape[1] // batch_size
        first_frame_video_tokens = (
            latent_dict['noisy_latents'].shape[-2] // self.patch_size[1]
        ) * (
            latent_dict['noisy_latents'].shape[-1] // self.patch_size[2]
        )
        for block_index, block in enumerate(self.blocks):
            hidden_states = block(hidden_states,
                                         ...
                                         update_cache=False)
            if self.action_head_type == "separate":
                per_sample_video = hidden_states[
                    :, : latent_hidden_states.shape[1]
                ].reshape(batch_size, latent_tokens_per_sample, self.inner_dim)
                video_context = per_sample_video[:, :first_frame_video_tokens]
                action_head_states = self.action_blocks[block_index](
                    action_head_states,
                    video_context,
                    action_head_text,
                    action_head_timestep_proj,
                    conditioning_states=action_head_conditioning,
                    action_key_mask=action_head_key_mask,
                )
        ...
        latent_hidden_states, action_hidden_states, _, _ = torch.split(hidden_states, split_list, dim=1)
        ...
        if self.action_head_type == "separate":
            action_head_modulation = (
                self.action_head_scale_shift_table[None]
                + action_head_temb[:, :, None, ...]
            )
            action_shift, action_scale = rearrange(
                action_head_modulation, 'b l n c -> b n l c'
            ).chunk(2, dim=1)
            action_head_states = (
                self.action_head_norm_out(action_head_states.float())
                * (1.0 + action_scale.squeeze(1))
                + action_shift.squeeze(1)
            ).type_as(action_head_states)
            action_hidden_states = self.action_head_proj_out(
                action_head_states[:, :action_token_count]
            )
        else:
    ...
    def _action_head_time_embed(self, timesteps, height, width, dtype):
        expanded = torch.repeat_interleave(timesteps, height * width, dim=1)
        temb, projection = self.action_head_condition_embedder(expanded, dtype=dtype)
        return temb, projection.unflatten(2, (6, -1))
    ...
    def forward(
        ...
        if self.action_head_type == "separate" and action_mode:
            return self._forward_separate_action_head_inference(
                input_dict,
                cache_name=cache_name,
                history=history,
            )
        ...
        action_video_contexts = []
        first_frame_video_tokens = (
            input_dict['noisy_latents'].shape[-2] // self.patch_size[1]
        ) * (
            input_dict['noisy_latents'].shape[-1] // self.patch_size[2]
        )
        for block in self.blocks:
            latent_hidden_states = block(latent_hidden_states,
                                         ...
                                         cache_name=cache_name,
                                         history=history)
            if self.action_head_type == "separate" and not action_mode:
                action_video_contexts.append(
                    latent_hidden_states[:, :first_frame_video_tokens]
                )
        if action_video_contexts:
            self._action_video_context_cache[cache_name] = action_video_contexts
        temb_scale_shift_table = self.scale_shift_table[None] + temb[:, :, None, ...]
        shift, scale = rearrange(temb_scale_shift_table,
                                 'b l n c -> b n l c').chunk(2, dim=1)
    ...
    def _forward_separate_action_head_inference(
        self, input_dict, *, cache_name, history
    ):
        if history:
            self._action_state_cache[cache_name] = {
                "value": input_dict["noisy_latents"].detach(),
                "timesteps": input_dict["timesteps"].detach(),
            }
            batch = input_dict["noisy_latents"].shape[0]
            tokens = math.prod(input_dict["noisy_latents"].shape[-3:])
            return torch.zeros(
                (batch, tokens, self.config.action_dim),
                device=input_dict["noisy_latents"].device,
                dtype=input_dict["noisy_latents"].dtype,
            )

        video_contexts = self._action_video_context_cache.get(cache_name)
        if not video_contexts:
            raise RuntimeError(
                "Separate action head requires video context. Compute the observation "
                "KV/context cache before action inference."
            )
        noisy_actions = input_dict["noisy_latents"]
        batch = noisy_actions.shape[0]
        action_values = rearrange(noisy_actions, 'b c f h w -> b (f h w) c')
        ...
        state_cache = self._action_state_cache.get(cache_name)
        if state_cache is not None and state_cache["value"].shape[0] == batch:
            state_values = rearrange(
                state_cache["value"], 'b c f h w -> b (f h w) c'
            )
            action_conditioning = self.action_head_embedder(state_values)
            key_masks.append(
                torch.ones(
                    state_values.shape[:2],
                    device=state_values.device,
                    dtype=torch.bool,
                )
            )
        action_states = self.action_head_embedder(action_values)
        action_key_mask = torch.cat(key_masks, dim=1)

        action_temb, action_projection = self._action_head_time_embed(
            input_dict["timesteps"],
            noisy_actions.shape[-2],
            noisy_actions.shape[-1],
            action_states.dtype,
        )
        text_states = self.action_head_condition_embedder.text_embedder(
            input_dict["text_emb"]
        )

        for block, video_context in zip(self.action_blocks, video_contexts):
            action_states = block(
                action_states,
                video_context,
                text_states,
                action_projection,
                conditioning_states=action_conditioning,
                action_key_mask=action_key_mask,
            )
        modulation = self.action_head_scale_shift_table[None] + action_temb[:, :, None]
        shift, scale = rearrange(modulation, 'b l n c -> b n l c').chunk(2, dim=1)
        action_states = (
            self.action_head_norm_out(action_states.float())
            * (1.0 + scale.squeeze(1))
            + shift.squeeze(1)
        ).type_as(action_states)
        return self.action_head_proj_out(action_states[:, :action_count])
