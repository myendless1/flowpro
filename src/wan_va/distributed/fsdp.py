...
def apply_ac(model):
    ...
    for layer_id, transformer_block in enumerate(getattr(model, "action_blocks", [])):
        transformer_block = ptd_checkpoint_wrapper(
            transformer_block, preserve_rng_state=False
        )
        model.action_blocks[layer_id] = transformer_block
...
def shard_model(model,
    ...
    for block in getattr(model, "action_blocks", []):
        fully_shard(block, **fsdp_config)
