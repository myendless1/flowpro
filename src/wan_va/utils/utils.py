...
def get_mesh_id(
    ...
    if action:
        if action_ff_override is None:
            ff_offset = (torch.ones([h]).cumsum(0) / (h + 1)).view(1, -1, 1)
            ff = ff + ff_offset
        else:
            ff = torch.full_like(ff, action_ff_override)
...
def get_state_history_grid_id(
    ...
    if action_per_frame <= 0:
        raise ValueError(
            f"action_per_frame must be positive, got {action_per_frame}"
        )

    unit = 1.0 / float(action_per_frame + 1)
    ff = -torch.arange(
        history_len,
        0,
        -1,
    ...
    hh = torch.full((history_len,), -1.0, dtype=dtype, device=device)
    ww = torch.full((history_len,), -1.0, dtype=dtype, device=device)
    tt = torch.full((history_len,), float(t), dtype=dtype, device=device)
    return torch.stack([ff, hh, ww, tt], dim=0)
